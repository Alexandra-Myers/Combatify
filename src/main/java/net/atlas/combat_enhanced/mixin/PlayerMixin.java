...
import net.atlas.combat_enhanced.item.NewAttributes;
import net.atlas.combat_enhanced.util.UtilClass;
...
public abstract class PlayerMixin extends LivingEntity implements PlayerExtensions, LivingEntityExtensions {
	...
	public void readAdditionalSaveData(CompoundTag nbt, CallbackInfo ci) {
		...
		Objects.requireNonNull(player.getAttribute(Attributes.ATTACK_DAMAGE)).setBaseValue(!CombatEnhanced.CONFIG.fistDamage() ? 2 : 1);
	}
	...
	private static double changeAttack(double constant) {
		return !CombatEnhanced.CONFIG.fistDamage() ? 2 : 1;
	}
	...
	public void injectCrit(Entity target, CallbackInfo ci, @Local(ordinal = 0) LocalFloatRef attackDamage, @Local(ordinal = 1) final float attackDamageBonus, @Local(ordinal = 2)LocalBooleanRef bl3) {
		...
		if(!CombatEnhanced.CONFIG.sprintCritsEnabled()) {
			isCrit &= !isSprinting();
		}
	...
	public void resetAttackStrengthTicker(boolean hit) {
		...
		if(!CombatEnhanced.CONFIG.attackSpeed()) {
	...
	public double getAttackRange(float baseTime) {
		...
		int chargedBonus = 0;
		double baseAttackRange = CombatEnhanced.CONFIG.attackReach() ? 0 : 0.5;
	...
	public boolean attackSpeedsMaxed() {
		CombatEnhancedConfig c = CombatEnhanced.CONFIG;
