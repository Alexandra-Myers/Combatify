...
import net.atlas.combat_enhanced.enchantment.CustomEnchantmentHelper;
...
public abstract class LivingEntityMixin extends Entity implements LivingEntityExtensions {
	...
	public void blockedByShield(LivingEntity target, CallbackInfo ci) {
		...
		if (CombatEnhanced.CONFIG.piercer()) {
		...
		if (bl && blockingItem instanceof IShieldItem shieldItem && shieldItem.getBlockingType().canBeDisabled()) {
			...
			float damage = CombatEnhanced.CONFIG.shieldDisableTime() + (float) CustomEnchantmentHelper.getChopping(((LivingEntity) (Object)this)) * CombatEnhanced.CONFIG.cleavingDisableTime();
			if(CombatEnhanced.CONFIG.defender()) {
				damage -= CustomEnchantmentHelper.getDefense(target) * CombatEnhanced.CONFIG.defenderDisableReduction();
			}
	...
	public int changeIFrames(int constant, @Local(ordinal = 0) final DamageSource source, @Local(ordinal = 0) final float amount) {
		...
		if (source.is(DamageTypeTags.IS_PROJECTILE) && !CombatEnhanced.CONFIG.projectilesHaveIFrames()) {
			invulnerableTime = 0;
		}

		if (source.is(DamageTypes.MAGIC) && !CombatEnhanced.CONFIG.magicHasIFrames()) {
			invulnerableTime = 0;
		}
	...
	public void injectEatingInterruption(DamageSource source, float f, CallbackInfoReturnable<Boolean> cir) {
		if(thisEntity.isUsingItem() && thisEntity.getUseItem().isEdible() && !source.is(DamageTypeTags.IS_FIRE) && !source.is(DamageTypeTags.WITCH_RESISTANT_TO) && !source.is(DamageTypeTags.IS_FALL) && CombatEnhanced.CONFIG.eatingInterruption()) {
	...
	public void modifyKB(LivingEntity instance, double d, double e, double f, @Local(ordinal = 0) final DamageSource source) {
		if ((CombatEnhanced.CONFIG.fishingHookKB() && source.getDirectEntity() instanceof FishingHook) || (!source.is(DamageTypeTags.IS_PROJECTILE) && CombatEnhanced.CONFIG.midairKB())) {
	...
	public ItemStack getBlockingItem() {
		...
		if (thisLivingEntity.isUsingItem() && !thisLivingEntity.getUseItem().isEmpty()) {
			...
			for(InteractionHand hand : InteractionHand.values()) {
				...
				boolean bl = CombatEnhanced.CONFIG.shieldOnlyWhenCharged() && thisLivingEntity instanceof Player player && player.getAttackStrengthScale(1.0F) < 1.95F && blockingItem instanceof IShieldItem shieldItem && shieldItem.getBlockingType().requireFullCharge();
