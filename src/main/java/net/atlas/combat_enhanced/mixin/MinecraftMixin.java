...
public abstract class MinecraftMixin implements IMinecraft {
	...
	public void checkIfCrouch(MultiPlayerGameMode instance, Player player) {
		...
		boolean bl = CombatEnhanced.CONFIG.shieldOnlyWhenCharged() && player.getAttackStrengthScale(1.0F) < 1.95F && blockingItem instanceof IShieldItem shieldItem && shieldItem.getBlockingType().requireFullCharge();
	...
	public void redirectReset(LocalPlayer player) {
		...
		if(result != null && CombatEnhanced.CONFIG.refinedCoyoteTime()) {
		...
		if (missTime <= 0) {
			...
			if (!this.player.isUsingItem()) {
				if (bl && this.hitResult != null && this.hitResult.getType() == HitResult.Type.BLOCK) {
					this.retainAttack = false;
				} else if (bl && ((PlayerExtensions) this.player).isAttackAvailable(-1.0F) && ((IOptions) options).autoAttack().get() && CombatEnhanced.CONFIG.autoAttackAllowed()) {
					this.startAttack();
					ci.cancel();
				}
			}
	...
	public InteractionResult addRequirement(MultiPlayerGameMode instance, LocalPlayer localPlayer, InteractionHand interactionHand, BlockHitResult blockHitResult, @Local ItemStack stack) {
		if(CombatEnhanced.CONFIG.shieldOnlyWhenCharged() && localPlayer.getAttackStrengthScale(1.0F) < 1.95F && stack.getItem() instanceof IShieldItem shieldItem && shieldItem.getBlockingType().requireFullCharge()) {
	...
	public InteractionResult addRequirement1(MultiPlayerGameMode instance, Player player, InteractionHand interactionHand, @Local ItemStack stack) {
		if(CombatEnhanced.CONFIG.shieldOnlyWhenCharged() && player.getAttackStrengthScale(1.0F) < 1.95F && stack.getItem() instanceof IShieldItem shieldItem && shieldItem.getBlockingType().requireFullCharge()) {
			return InteractionResult.PASS;
		}
