...
public record CombatifyCritImpl(boolean allowsSprint, float minimumBaseCharge, float minimumFullCharge, float critMult, float chargedCritMult) implements CritImpl {
	...
	public static final MapCodec<CombatifyCritImpl> CODEC = RecordCodecBuilder.mapCodec(instance ->
		instance.group(Codec.BOOL.optionalFieldOf("allows_sprint", true).forGetter(CombatifyCritImpl::allowsSprint),
				Codec.floatRange(-1, 2).optionalFieldOf("minimum_base_charge", 0.9F).forGetter(CombatifyCritImpl::minimumBaseCharge),
				Codec.floatRange(-1, 2).optionalFieldOf("minimum_full_charge", 1.95F).forGetter(CombatifyCritImpl::minimumFullCharge),
				Codec.floatRange(0, Float.MAX_VALUE).optionalFieldOf("crit_multiplier", 1.25F).forGetter(CombatifyCritImpl::critMult),
				Codec.floatRange(0, Float.MAX_VALUE).optionalFieldOf("charged_crit_multiplier", 1.5F).forGetter(CombatifyCritImpl::chargedCritMult))
