...
public record PatchedDataComponentMapWrapper(RegistryOps<@NonNull Tag> ops, ResourceKey<@NonNull Registry<@NonNull DataComponentType<?>>> reg, PatchedDataComponentMap value) implements DataComponentMapWrapper<PatchedDataComponentMap> {
	public <T> void set(String type, String value) {
		@SuppressWarnings("unchecked") DataComponentType<@NonNull T> dataComponentType = (DataComponentType<@NonNull T>) ops().getter(reg()).orElse(BuiltInRegistries.DATA_COMPONENT_TYPE).getOrThrow(ResourceKey.create(reg(), Identifier.parse(type))).value();
