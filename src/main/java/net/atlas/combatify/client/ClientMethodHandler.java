...
public class ClientMethodHandler {
	...
	public static void swingWeaponDown(ModelPart rightArm, ModelPart leftArm, HumanoidArm mainArm, float attackTime, float ageInTicks, float headXRot) {
		float h = Mth.sin(attackTime * 3.1415927F);
		float i = Mth.sin((1.0F - (1.0F - attackTime) * (1.0F - attackTime)) * 3.1415927F);
		rightArm.zRot = 0.0F;
		leftArm.zRot = 0.0F;
	...
	public static void animateZombieArms(ModelPart leftArm, ModelPart rightArm, HumanoidArm mainArm, float attackTime, float ageInTicks, float headXRot) {
		float h = Mth.sin(attackTime * 3.1415927F);
		float i = Mth.sin((1.0F - (1.0F - attackTime) * (1.0F - attackTime)) * 3.1415927F);
		rightArm.zRot = 0.0F;
		leftArm.zRot = 0.0F;
		boolean isRightHanded = mainArm == HumanoidArm.RIGHT;
