...
public class FoodHelperMixin {
	...
	private static int modifyMinHunger(int i, int j, Operation<Integer> original, @Local(ordinal = 0) FoodData foodData, @Local(ordinal = 0) Player player, @Local(ordinal = 0, argsOnly = true) FoodProperties foodProperties) {
		...
		return (int) Combatify.CONFIG.getFoodImpl().execGetterFunc(original.call(i, j), "estimateNewFoodLevel(foodData, player, foodProperties)", new FoodDataWrapper(foodData), new PlayerWrapper<>(player), new FoodPropertiesWrapper(foodProperties));
	}
	...
	private static float modifyMinHunger(float a, float b, Operation<Float> original, @Local(ordinal = 0) FoodData foodData, @Local(ordinal = 0) Player player, @Local(ordinal = 0, argsOnly = true) FoodProperties foodProperties) {
		...
		return (float) Combatify.CONFIG.getFoodImpl().execGetterFunc(original.call(a, b), "estimateNewSaturationLevel(foodData, player, foodProperties)", new FoodDataWrapper(foodData), new PlayerWrapper<>(player), new FoodPropertiesWrapper(foodProperties));
	}
	...
	private static float changeExhaustion(int foodLevel, float saturationLevel, float exhaustionLevel, Operation<Float> original) {
		...
		if (Combatify.CONFIG.getFoodImpl().execFunc("shouldOverrideAppleSkin()"))
			return (float) Combatify.CONFIG.getFoodImpl().execGetterFunc(0.0, "editAppleSkinHealthGained(foodLevel, saturationLevel, exhaustionLevel)", foodLevel, saturationLevel, exhaustionLevel);
