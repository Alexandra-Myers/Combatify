...
public class FoodHelperMixin {
	...
	private static int modifyMinHunger(int i, int j, Operation<Integer> original, @Local(ordinal = 0) FoodData foodData, @Local(ordinal = 0) Player player, @Local(ordinal = 0, argsOnly = true) FoodProperties foodProperties) {
		...
		return (int) Combatify.CONFIG.getFoodImpl().execFoodGetterFunc(original.call(i, j), foodData, player, "estimateNewFoodLevel(foodData, player, foodProperties)", new JSImpl.Reference("foodProperties", foodProperties));
	}
	...
	private static float modifyMinHunger(float a, float b, Operation<Float> original, @Local(ordinal = 0) FoodData foodData, @Local(ordinal = 0) Player player, @Local(ordinal = 0, argsOnly = true) FoodProperties foodProperties) {
		...
		return (float) Combatify.CONFIG.getFoodImpl().execFoodGetterFunc(original.call(a, b), foodData, player, "estimateNewSaturationLevel(foodData, player, foodProperties)", new JSImpl.Reference("foodProperties", foodProperties));
	}
	...
	private static int changeConst2(int original, @Local(ordinal = 0, argsOnly = true) int foodLevel, @Local(ordinal = 0, argsOnly = true) float saturationLevel, @Local(ordinal = 1, argsOnly = true) float exhaustionLevel) {
		if(Combatify.CONFIG.getFoodImpl().execFunc("canFastHealRaw(foodLevel, saturationLevel, exhaustionLevel)", new JSImpl.Reference("foodLevel", foodLevel), new JSImpl.Reference("saturationLevel", saturationLevel), new JSImpl.Reference("exhaustionLevel", exhaustionLevel)) || Combatify.state.equals(Combatify.CombatifyState.VANILLA))
	...
	private static float changeExhaustion(int foodLevel, float saturationLevel, float exhaustionLevel, Operation<Float> original) {
		...
		if (Combatify.CONFIG.getFoodImpl().execFunc("shouldOverrideAppleSkin()")) return (float) Combatify.CONFIG.getFoodImpl().execGetterFunc(0.0, "editAppleSkinHealthGained(foodLevel, saturationLevel, exhaustionLevel)", new JSImpl.Reference("foodLevel", foodLevel), new JSImpl.Reference("saturationLevel", saturationLevel), new JSImpl.Reference("exhaustionLevel", exhaustionLevel));
