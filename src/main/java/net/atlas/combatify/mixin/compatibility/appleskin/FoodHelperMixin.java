...
public class FoodHelperMixin {
	...
	private static int modifyMinHunger(int i, int j, Operation<Integer> original, @Local(ordinal = 0) FoodData foodData, @Local(ordinal = 0, argsOnly = true) Player player, @Local(ordinal = 0, argsOnly = true) ConsumableFood food) {
		...
		return (int) Combatify.CONFIG.getFoodImpl().execGetterFunc(original.call(i, j), "estimateNewFoodLevel(foodData, player, foodProperties)", new FoodDataWrapper(foodData), new PlayerWrapper<>(player), new FoodPropertiesWrapper(food.food()));
	}
	...
	private static float modifyMinHunger(float a, float b, Operation<Float> original, @Local(ordinal = 0) FoodData foodData, @Local(ordinal = 0, argsOnly = true) Player player, @Local(ordinal = 0, argsOnly = true) ConsumableFood food) {
		...
		return (float) Combatify.CONFIG.getFoodImpl().execGetterFunc(original.call(a, b), "estimateNewSaturationLevel(foodData, player, foodProperties)", new FoodDataWrapper(foodData), new PlayerWrapper<>(player), new FoodPropertiesWrapper(food.food()));
	}
	...
	private static int changeConst2(int original, @Local(ordinal = 0, argsOnly = true) int foodLevel, @Local(ordinal = 0, argsOnly = true) float saturationLevel, @Local(ordinal = 1, argsOnly = true) float exhaustionLevel) {
		if(Combatify.CONFIG.getFoodImpl().execFunc("canFastHealRaw(foodLevel, saturationLevel, exhaustionLevel)", foodLevel, saturationLevel, exhaustionLevel) || Combatify.getState().equals(Combatify.CombatifyState.VANILLA))
	...
	private static float changeExhaustion(int foodLevel, float saturationLevel, float exhaustionLevel, Operation<Float> original) {
		...
		if (Combatify.CONFIG.getFoodImpl().execFunc("shouldOverrideAppleSkin()")) return (float) Combatify.CONFIG.getFoodImpl().execGetterFunc(0.0, "editAppleSkinHealthGained(foodLevel, saturationLevel, exhaustionLevel)", foodLevel, saturationLevel, exhaustionLevel);
